...
let grid = document.querySelector(".gridGallery");

fetch("https://picsum.photos/v2/list")
.then((response) => response.json())
.then((data) => {
    
data.forEach((singleGallery) => {
    grid.innerHTML += 
`<div>
        <img src="${singleGallery.download_url}" alt="gallerie img"> 
        <h2>${singleGallery.author}</h2>
        <button><a href="${singleGallery.url}">See more</a></button>
</div>`;
    });
);
